def single_text_style_extraction_prompt(text: str, style_target: str = None) -> str:
    """Generated prompt template function."""
    style_target = style_target or "not specified"
...
${styleTarget && styleTarget.trim().toLowerCase() !== 'all' && styleTarget.trim() !== '' ? `Focus specifically on the writing style of the character or person named "{style_target}". If this person's dialogue or narration isn't present, analyze the overall style and note that the target was not found.` : `Analyze the overall writing style of the entire text.`}
...
def chunk_style_analysis_prompt(text: str, style_target: str = None) -> str:
    """Generated prompt template function."""
    style_target = style_target or "not specified"
    text = text or "not specified"
    return f"""
...
${styleTarget && styleTarget.trim().toLowerCase() !== 'all' && styleTarget.trim() !== '' ? `Focus specifically on the writing style of the character or person named "{style_target}". If this person's dialogue or narration isn't present, analyze the overall style and note that the target was not found.` : `Analyze the overall writing style of the entire text segment.`}
...
def reduce_style_analyses_prompt(text: str, style_target: str = None) -> str:
    """Generated prompt template function."""
    style_target = style_target or "not specified"
    text = text or "not specified"
    return f"""
...
${styleTarget && styleTarget.trim().toLowerCase() !== 'all' && styleTarget.trim() !== '' ? `The final model should focus on the style of "{style_target}".` : `The final model should represent the overall style.`}
