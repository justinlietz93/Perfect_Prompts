...
// AgentDesignerSettings represents the configuration for agent design
...
// AgentDesignerPromptTemplate generates a comprehensive agent design prompt
func AgentDesignerPromptTemplate(settings AgentDesignerSettings) string {
	isSingleAgent := settings.SystemType == "singleAgent"
	...
	if isSingleAgent {
		...
		persona = "You are an expert multi-agent systems architect. Your task is to design a robust and scalable agentic system based on a user's high-level goal and provided configuration."
	}
...
1.  **Analyze Goal:** Deconstruct the user's goal: "%s".
...
6.  **Synthesize Artifacts:**
    ...
    *   **designFlowDiagram:** Create the Mermaid.js `+"`graph TD`"+` that visually represents the agent's process in the `+"`<DESIGN_FLOW_DIAGRAM>`"+` block.`, settings.Goal)
		...
		webSearch := settings.Capabilities["webSearch"]
		emailAccess := settings.Capabilities["emailAccess"]
		fileIO := settings.Capabilities["fileIO"]
		codeExecution := settings.Capabilities["codeExecution"]
...
3.  **Define Agents & Tools:**
    ...
    *   Define the tools required to accomplish the goal, especially considering the user-selected capabilities: Web Search (%v), Email (%v), File I/O (%v), Code Execution (%v).
...
5.  **Synthesize Artifacts:**
    ...
    *   **designFlowDiagram:** Create a Mermaid.js `+"`graph TD`"+` that visually represents the process flow in the `+"`<DESIGN_FLOW_DIAGRAM>`"+` block.`, 
			settings.Goal, webSearch, emailAccess, fileIO, codeExecution)
	}
...
`, persona, designPrinciples, pipeline, settings.Goal, settings.SystemType, settings.Provider, settings.Trigger)
}