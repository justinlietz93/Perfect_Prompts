...
// RequestSplitterPlanningPrompt generates a prompt template
func RequestSplitterPlanningPrompt(spec string, settings string) string {
	return fmt.Sprintf(`
...
// RequestSplitterGenerationPrompt generates a prompt template
func RequestSplitterGenerationPrompt(project_context string, current_prompt_title string, current_prompt_id string, completed_prompt_titles string, title string) string {
	return fmt.Sprintf(`
...
---
[#${currentPromptId.toString().padStart(2, '0')}] %s

**PROJECT CONTEXT**
- **Application:** ${projectContext.name}
- **Architecture:** ${projectContext.architecture}
- **Invariants:**
${projectContext.invariants.map(inv => `  - %s`).join('\n')}
...
**FEATURE REQUEST**
- **Modify the existing codebase** to implement exactly one new feature: **%s**.
...
`, current_prompt_title, inv, current_prompt_title)
}
