pub fn get_persona_directive(settings: &str, logical consistency: &str, and empirical evidence. _justify steps with established physical laws or logical axioms.";
...
export const generate_critique_prompt = (
    main_goal: &str, step_to_critique: &str, settings: Option<&str>, risks: &str, or unstated assumptions?
...
export const generate_refine_prompt = (
    main_goal: Option<&str>, original_step: &str, critique: &str, settings: &str, and it has received the following critique.
...
export const generate_synthesize_prompt = (
    main_goal: &str, alternatives: &str, settings: &str, i: &str) -> String {
    format!(r#"
--- ALTERNATIVE ${{i + 1}} ---
${{alt}}
`).join('\n\n');
...
"#, alt.unwrap_or("not specified"), persona.unwrap_or("not specified"), main_goal.unwrap_or("not specified"), alternatives_text.unwrap_or("not specified"))
}
